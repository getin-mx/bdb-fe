(function ($, w, undefined) {
	...
	function pageInfo(ft) {
		...
		this.pageCallback = ft.pageCallback;
	}
	...
	function Paginate() {
		...
		p.init = function (ft) {
			if (ft.options.paginate === true) {
				...
				$(ft.table)
					.unbind('.paging')
					.bind({
						'footable_initialized.paging footable_row_removed.paging footable_redrawn.paging footable_filtered.paging': function () {
							p.setupPaging();
						}
					})
					.bind({
						 'footable_sorted.paging': function () {
							p.sortPaging();
						 }
					})
		...
		p.sortPaging = function() {
			...
			var $table = $(ft.table), data = $table.data();
			p.paginate(ft, data.currentPage);
		};
		...
		p.createPages = function (ft, tbody) {
			...
			// Add all the pages acording to the 'footable-record-count' data value
			if( info.recordCount !== undefined )  {
				var neededPages = Math.floor(info.recordCount  / info.pageSize);
				if( info.recordCount % info.pageSize > 0 
					&& info.recordCount > info.pageSize 
					&& info.pages.length == 1) neededPages++;
				
				for( var i = info.pages.length; i < neededPages; i++ ) {
					info.pages.push(new Array());
				}
			}
		...
		p.paginate = function (ft, newPage) {
			...
			// if (info.currentPage !== newPage) {
				...
				if(info.pageCallback !== undefined) {
					var $table = $(ft.table), data = $table.data();
					var sort = null;
					try {
						var columnIndex = data.sortColumnIndex;
						var ascending = data.sortAscending;
						sort = data.footable.columns[columnIndex].sort.name;
					...
					info.pageCallback(ft, newPage, sort, function(ft, newPage ) {
						info.currentPage = newPage;
						info.control.find('li').removeClass('active disabled');
						var $nav = $(ft.table).find(ft.pageInfo.pageNavigation);
						if( info.limit )
							p.createLimited($nav, info, (newPage - (newPage % info.limitNavigation)));
		...
		p.fillPage = function (ft, tbody, pageNumber) {
			...
			if(info.pageCallback !== undefined) {
				$(ft.pageInfo.pages[0]).each(function () {
					p.showRow(this, ft);
				});
			} else {
				tbody.find('> tr').hide();
