/**
 * BrandsCtrl - controller
 */
function BrandsCtrl($scope, $http, $location, CommonsService, AuthenticationService) {
	...
	$scope.refresh = function() {
		...
		$scope.loadingRefresh = true;
		$http.get(CommonsService.getUrl('/brand') 
	...
	$scope.fillTableActive = function(data) {
		CommonsService.paginateCommonTable($scope, 'brand', data, 0);
	}

	$scope.fillTableInactive = function(data) {
		CommonsService.paginateCommonTable($scope, 'brand', data, 1);
	}
